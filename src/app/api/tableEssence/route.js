...
export async function GET(request) {
  try {
    ...
    const resultsDataBoxes = dataBoxes.data.erc1155Tokens.results.map(
      (result) => ({
        ...result,
        ...
        minPriceUsd: result.minPrice
          ...
          : "Not sale",
        requireLevel:
          result.attributes["requires level"] &&
          result.attributes["requires level"][0],
      }),
    );
    ...
    function findPreviousLevelItem(name) {
      ...
      if (!currentItem) {
        return "N/A"; // Retorna "N/A" si no se encuentra el ítem actual
      }

      // Buscar el valor del "requires level" dentro del array de attributes
      const requiresLevelObj = currentItem.attributes.find(
        (attribute) => attribute["requires level"],
      );
      ...
      if (requiresLevelObj) {
        ...
        if (isNaN(currentLevel)) {
          return "N/A"; // Retorna "N/A" si el nivel no es válido
        }

        // Si el nivel es 1, retornar "N/A"
        if (currentLevel === 1) {
          return "N/A";
        }
        ...
        const previousLevelItem = dataEssence.find((item) => {
          const prevLevelObj = item.attributes.find(
            (attribute) => attribute["requires level"],
          );
        ...
        // Validar si se encontró el ítem anterior
        return previousLevelItem ? previousLevelItem.minPriceUsd : "N/A"; // Devuelve el precio o "N/A"
      }

      return "N/A"; // Retorna "N/A" si no se encuentra el atributo "requires level"
    }
